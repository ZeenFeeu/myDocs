...
| tx_deposit_gas_exchange_ratio | 10 uTOP/Tgas                 | （0，正无穷） | 交易保证金抵扣gas的gasprice。                                | normal   |
...
| unlock_gas_staked_delay_time  | 8641个时钟块（24小时多10秒） | （0，正无穷） | 解锁gas抵押金delay到账的时间。<br/>避免投机者快进快出，让gas的分配剧烈波动。 | Normal   |
...
| min_auditor_votes              | 1,000,000           | （0,正无穷）  | auditor节点选举，的票总数最低不能低于1,000,000票，否则选举失败。 | normal   |
| dividend_ratio_change_interval | 14d                 | （0,正无穷）  | 分红比例修改时间间隔。                                       | normal   |
| min_mainnet_active_archives    | 1                   | [0，正无穷）  | 主网激活archive最低节点数。                                  |          |
| min_mainnet_active_auditors    | 128                 | [0，正无穷）  | 主网激活auditor最低节点数。                                  |          |
| min_mainnet_active_edges       | 1                   | [0，正无穷）  | 主网激活edge最低节点数。                                     |          |
| min_mainnet_active_votes       | 0                   | [0，正无穷）  | 主网激活最低总票数。                                         |          |
| min_mainnet_active_validators  | 512                 | [0，正无穷）  | 主网激活最低validator节点数。                                |          |

## 兑票与投票

| 参数名称              | 创世初始值 | 治理Range    | 字段/初始值说明                                              | 级别   |
| --------------------- | ---------- | ------------ | ------------------------------------------------------------ | ------ |
| min_stake_votes_num   | 10000      | [0，正无穷） | 1万票起兑，最低兑票数。                                      | normal |
| min_votes_num         | 0          | [0，正无穷） | 最低票数。<br/>在一笔交易中，给每个节点增加的票数和取消的票数必须大于该值。 | normal |
| max_vote_nodes_num    | 1000       | [0，正无穷） | 最大投票节点数。<br/>限制单个账户投票节点数，而不是单个交易投票节点数。 | normal |
...
| workload_report_min_txs       | 32         | [0，正无穷） | 工作量上报最低table数。<br/>低于该值表示零工作量，因为系统自己也会产生交易（比如工作量上报）会出块，只有内部交易应该属于”零工作量” | important |
...
| 参数名称                 | 创世初始值       | 治理Range | 参数/初始值说明          | 级别     |
| ------------------------ | ---------------- | --------- | ------------------------ | -------- |
| cgc_proposal_expire_time | 259200时钟块     |           | 提案超时时间。           | critical |
| tcc_member_number        | 7                |           | TCC成员数量。            | critical |
| min_cgc_proposal_deposit | 100,000,000 uTOP |           | normal提案质押金最低值。 |          |

## 选举

| 参数名称                                                     | 创世初始值                                  | 治理Range | 参数/初始值说明                                              | 级别     |
| ------------------------------------------------------------ | ------------------------------------------- | --------- | ------------------------------------------------------------ | -------- |
| rec_election_interval: "259211"                              | 60480时钟周期（逻辑时间7天：7*24*60*60/10） |           | REC group选举周期。                                          | normal   |
...
| archive_election_interval: "11"                              | 360时钟周期                                 |           | archive选举周期。                                            | normal   |
| cluster_election_interval: "1598"                            | 71时钟周期                                  |           | consensus cluster选举周期（创世前）。                        | normal   |
| auditor_group_count                                          | 2                                           |           | 上线后共2个auditor group。                                   | normal   |
| validator_group_count                                        | 4                                           |           | 验证者group数，即shard数。<br/>上线后共4个validator group。  | critical |
